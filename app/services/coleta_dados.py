...
def _salvar_exercicios(plano: Dict[str, Any], db: Session) -> int:
    ...
    for dia_info in dias_treino:
        ...
        for exercicio_info in exercicios_dia:
            ...
            exercicio_existente = db.query(Exercicio).filter(
                Exercicio.nome == nome
            ).first()
...
def _salvar_refeicoes(plano: Dict[str, Any], db: Session) -> int:
    ...
    for tipo_key, tipo_label in tipos_refeicao.items():
        ...
        for chave_original, refeicao_info in opcoes.items():
            ...
            refeicao_existente = db.query(Refeicao).filter(
                Refeicao.nome == nome
            ).first()
...
def obter_estatisticas_coleta(db: Session) -> Dict[str, Any]:
    try:
        ...
        total_refeicoes = db.query(Refeicao).filter(
            Refeicao.rotina_id.is_(None)
        ).count()
        
        # Agrupar por tipo
        pre_treino = db.query(Refeicao).filter(
            Refeicao.tipo == "pre_treino",
            Refeicao.rotina_id.is_(None)
        ).count()
        
        pos_treino = db.query(Refeicao).filter(
            Refeicao.tipo == "pos_treino",
            Refeicao.rotina_id.is_(None)
        ).count()
        
        # Agrupar por nível
        economica = db.query(Refeicao).filter(
            Refeicao.nivel == "economica",
            Refeicao.rotina_id.is_(None)
        ).count()
        
        equilibrada = db.query(Refeicao).filter(
            Refeicao.nivel == "equilibrada",
            Refeicao.rotina_id.is_(None)
        ).count()
        
        premium = db.query(Refeicao).filter(
            Refeicao.nivel == "premium",
            Refeicao.rotina_id.is_(None)
        ).count()
